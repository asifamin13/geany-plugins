...
#include "bracketcolors-ignore-styles.h"
...
/* --------------------------------- CONSTANTS ------------------------------ */
     ...
     * discovered from trial and error. only used if lexer doesnt
     * support style metadata or not found in autogenerated mapping.
     * only used as a last resort
     */

    static const std::set<guint> sDefaultIgnoreStyles {
        1, 2, 3, 4, 5, 7, 9 //  these seem to be consistent
    };
...
/* ----------------------------------- TYPES -------------------------------- */
    ...
    struct BracketColorsData {
        ...
        BracketColorsData() :
            ...
            init(FALSE),
            ignoreStyles(sDefaultIgnoreStyles)
        {
...
----------------------------------------------------------------------------- */
{
    ...
    if (data->ignoreStyles.find(style) != data->ignoreStyles.end()) {
        return TRUE;
    }
    return FALSE;
}
...
// -----------------------------------------------------------------------------
    static void get_ignore_syles(
        BracketColorsData *data
    )
/*
    check lexer for styles that should be ignored when considering bracket map
----------------------------------------------------------------------------- */
{
    // tags which should not be ignored
    static const std::set<std::string> sFilters = {
        "default",
        "identifier",
        "operator"
    };

    static std::map<gint, std::set<guint> > sStyleCache;

    ScintillaObject *sci = data->doc->editor->sci;
    gint lexer = sci_get_lexer(sci);

    if (sStyleCache.find(lexer) != sStyleCache.end()) {
        // g_debug(
        //     "%s: Using style cache for lexer %d",
        //     __FUNCTION__, lexer
        // );
        data->ignoreStyles = sStyleCache.at(lexer);
        return;
    }

    gint namedStyles = SSM(
        sci, SCI_GETNAMEDSTYLES,
        BC_NO_ARG,
        BC_NO_ARG
    );

    g_debug(
        "%s: Lexer %d has %d named styles",
        __FUNCTION__, lexer, namedStyles
    );

    if (namedStyles > 0) {

        /*
         * Look through style tags and ignore everything that isnt
         * default, identifier, or operator
         */

        gchar buff[256];
        data->ignoreStyles.clear();

        for (gint i = 0; i < namedStyles; i++) {

            SSM(sci, SCI_NAMEOFSTYLE, i, (sptr_t) buff);
            std::string name(buff);

            SSM(sci, SCI_TAGSOFSTYLE, i, (sptr_t) buff);
            std::string tags(buff);

            if (tags.size()) {
                // to lowercase, just in case
                std::transform(
                    tags.begin(), tags.end(), tags.begin(),
                    [](unsigned char c){ return std::tolower(c); }
                );
                //g_debug("checking style %d with tags '%s' ...", i, tags.c_str());
                bool ignoreStyle = true;
                for (const auto &it : sFilters) {
                    if (tags.find(it) != std::string::npos) {
                        ignoreStyle = false;
                        break;
                    }
                }
                if (ignoreStyle) {
                    g_debug(
                        "%s: ... Ignoring style %d with tags '%s'",
                        __FUNCTION__, i, tags.c_str()
                    );
                    data->ignoreStyles.insert(i);
                }
            }
        }
    }
        ...
        /*
         * Old lexer, see if we can find it in the autogenerated ignore mapping
         */

        auto it = sGeneratedIgnoreStyles.find(lexer);
        if (it != sGeneratedIgnoreStyles.end()) {
            g_debug("%s: Using generated ignore styles", __FUNCTION__);
            data->ignoreStyles = it->second;
        }
        else {
            g_debug(
                "%s: Dont have style information, using defaults", __FUNCTION__
            );
        }
    }

    sStyleCache.insert(std::make_pair(lexer, data->ignoreStyles));
}
...
// -----------------------------------------------------------------------------
    static void on_document_filetype_set(
        GObject *obj,
        GeanyDocument *doc,
...
----------------------------------------------------------------------------- */
{
    g_debug("%s: handling new document filetype", __FUNCTION__);
    gpointer pluginData = plugin_get_document_data(geany_plugin, doc, sPluginName);
    if (pluginData != NULL) {
        BracketColorsData *data = reinterpret_cast<BracketColorsData *>(pluginData);
        get_ignore_syles(data);
    }
}
